...
function getaL(J,L,K,ms_m)
	...
	aL[1]=10*(J-aL[2]*τ0) - 3*(L-aL[2]*τ0) + 0.5*(K-aL[2]*τ0)
	aL[4]=-15*(J-aL[2]*τ0) + 5*(L-aL[2]*τ0) - 1*(K-aL[2]*τ0)
	aL[5]=6*(J-aL[2]*τ0) - 2*(L-aL[2]*τ0) + 0.5*(K-aL[2]*τ0)
...
function GenerateJLK()
	...
	write(io1,"index,Parameter Name,J (MeV),L (MeV),K (MeV),m*/m")
		...
		for l=eachindex(L)
			for k=eachindex(K)
				for mi=eachindex(ms_m)
					...
					write(io1,"$(count),J$(J[j])L$(L[l])K$(K[k])ms_m$(ms_m[mi])")
