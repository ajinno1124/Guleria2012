...
x_data,y_data=np.meshgrid(J_mesh,L_mesh,indexing='ij')
z_data=np.zeros([len(J_mesh),len(L_mesh)])
for i in range(0,len(J_mesh)):
	for j in range(0,len(L_mesh)):
		df_temp=df_best[abs(df_best["J (MeV)"]-J_mesh[i])<0.01]
		df_temp=df_temp[abs(df_best["L (MeV)"]-L_mesh[j])<0.01]
		print(df_temp)
		if len(df_temp)==0:
			z_data[i][j]=np.nan
		else:
...
plt.savefig("JL_best.pdf",dpi=300)
plt.savefig("JL_best.png",dpi=300)
#plt.show()
