...
f=open('JK_best.csv','w',encoding="utf-8")
...
df_best=pd.read_csv('JK_best.csv',comment='#')
print(df_best.head())

x_data,y_data=np.meshgrid(J_mesh,K_mesh,indexing='ij')
z_data=np.zeros([len(J_mesh),len(K_mesh)])
...
#plt.tight_layout()
plt.savefig("JK_best.pdf",dpi=300)
plt.savefig("JK_best.png",dpi=300)
#plt.show()
