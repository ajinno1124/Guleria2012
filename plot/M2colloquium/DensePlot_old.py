...
for i in range(1,21):
	df=pd.read_csv(f'../../data/Potential/Potential_{i}/DensityDep_{i}.csv',comment="#")
	ax.plot(df["density"],df["U"],color="r",linewidth=1)
...
ax.fill(np.append(df1["density"],df2["density"][::-1]),np.append(df1["U"],df2["U"][::-1]),label="GKW2",color=(0,0,1,0.08),edgecolor='black',zorder=10)
ax.fill(np.append(df3["density"],df4["density"][::-1]),np.append(df3["U"],df4["U"][::-1]),label="GKW3",color=(1,0,0,0.08),edgecolor='black',zorder=10)
...
plt.savefig("DensityDependence_old.pdf",dpi=300)
plt.savefig("DensityDependence_old.png",dpi=300)
