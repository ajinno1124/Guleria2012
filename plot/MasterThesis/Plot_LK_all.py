...
df_best=pd.read_csv('LK_best.csv',comment='#')
print(df_best.head())

x_data,y_data=np.meshgrid(L_mesh,K_mesh,indexing='ij')
z_data=np.zeros([len(L_mesh),len(K_mesh)])
for i in range(0,len(L_mesh)):
	for j in range(0,len(K_mesh)):
		df_temp=df_best[abs(df_best["L (MeV)"]-L_mesh[i])<0.01]
