...
f=open('mK_best.csv','w',encoding="utf-8")
...
df_best=pd.read_csv('mK_best.csv',comment='#')
print(df_best.head())

x_data,y_data=np.meshgrid(ms_m_mesh,K_mesh,indexing='ij')
z_data=np.zeros([len(ms_m_mesh),len(K_mesh)])
for i in range(0,len(ms_m_mesh)):
	for j in range(0,len(K_mesh)):
		df_temp=df_best[abs(df_best["m*/m"]-ms_m_mesh[i])<0.01]
		df_temp=df_temp[abs(df_best["K (MeV)"]-K_mesh[j])<0.01]
		print(df_temp)
		if len(df_temp)==0:
			z_data[i][j]=np.nan
		else:
...
#ax.set_xticks(np.array([0.6,0.7,0.8,0.9,1.0]), fontsize=14)
ax.set_yticks(np.array([0,100,200,300,400,500,600]), fontsize=14)
...
ax.tick_params(axis='x', which='both', direction='in',labelsize=14, labelrotation=-20)
...
#plt.tight_layout()
plt.savefig("mK_best.pdf",dpi=300)
plt.savefig("mK_best.png",dpi=300)
#plt.show()
