...
def BindingEnergyL(NParamType,LParamType):
	...
	f=open('BindingEnergyFernandez.csv','w',encoding="utf-8")
...
df=pd.read_csv('BindingEnergyFernandez.csv',comment='#')
d0=df[df["lLam"]==0]
d1=df[df["lLam"]==1]
d2=df[df["lLam"]==2]
d3=df[df["lLam"]==3]
...
ax.text(0.1,28,r'SKS3',{'color':'k','fontsize':14})
ax.text(0.1,26,r'SKSH3',{'color':'k','fontsize':14})
...
plt.savefig("BElam_Fernandez.pdf",dpi=300)
plt.savefig("BElam_Fernandez.png",dpi=300)
plt.show()
